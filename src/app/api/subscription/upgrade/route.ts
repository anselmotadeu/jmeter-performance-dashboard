 ...
 * cancela o agendamento e depois prossegue.
 *
...
export async function POST(request: Request) {
  try {
    ...
    if (!itemId || !currentPriceId) return Response.json({ error: 'Assinatura inválida.' }, { status: 400 });

    const hasCancelScheduled = stripeSub.cancel_at_period_end || stripeSub.cancel_at != null;

    if (hasCancelScheduled) {
      if (!reactivate) {
        // Front exibe modal de confirmação de reativação
      ...
      console.log(`[upgrade] Cancelamento agendado removido: ${sub.stripeSubscriptionId}`);
    }

    // Liberar schedule de downgrade pendente
    ...
    if (!periodEnd) return Response.json({ error: 'Não foi possível obter a data de renovação. Tente novamente.' }, { status: 500 });
    ...
    if (newUnits <= currentUnits) return Response.json({ error: 'O plano selecionado não representa um upgrade.' }, { status: 400 });
    ...
    // Checkout Session de pagamento único com CPF/CNPJ para NFS-e
