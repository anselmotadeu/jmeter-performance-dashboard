...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (session?.user) {
      ...
      // 5. Subscription direta no banco
      try {
        const raw = await db.query<{ id: string; status: string; trial_ends_at: Date | null; plan_slug: string }>(`
  ...
  // 6. Testar conexão Stripe (sem sessão real, sem custom_fields para simplificar)
  ...
  try {
    ...
    result.stripe_test = { success: true, session_id: checkoutSession.id };
  } catch (err) {
    result.stripe_test = {
      success: false,
